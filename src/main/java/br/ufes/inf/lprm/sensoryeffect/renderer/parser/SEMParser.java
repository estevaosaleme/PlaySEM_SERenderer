...
public class SEMParser implements Runnable {
	...
	public Vector<TimeLineAction> getTimeLineActions() {
	...
	public SEMParser(String newSensoryEffectMetadata, long newDuration, int newEventId) {
		timeLineActions = new Vector<TimeLineAction>();
		effectElements = new Vector<EffectElement>();
		sensoryEffectMetadata = newSensoryEffectMetadata;
		duration = newDuration;
	...
	public void run() {
		...
		if (SERendererBroker.debugMode){
			for (int i = 0; i < timeLineActions.size(); i++)
				System.out.println(timeLineActions.get(i).getOccurrenceTime() + ": " + SerialMessage.messageToString(timeLineActions.get(i).getSerialMessage()));
			System.out.println("Ready to start...");
			System.out.println("---");
		}
		
		if (eventId != 0) {
			HashMap<Vector<TimeLineAction>,Long> actionAndDuration = new HashMap<Vector<TimeLineAction>,Long>();
			actionAndDuration.put(timeLineActions, duration);
			SERendererBroker.service.getEventList().put(Integer.valueOf(eventId), actionAndDuration);
			...
			SERendererBroker.timeLine.setActions(timeLineActions, duration);
